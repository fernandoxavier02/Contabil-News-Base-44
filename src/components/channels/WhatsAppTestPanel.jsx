...
import { Send, Loader2, CheckCircle, XCircle, AlertCircle, MessageCircle } from "lucide-react";
import { News } from "@/api/entities";
import { sendToWhatsApp } from "@/api/functions";
...
export default function WhatsAppTestPanel({ configs = [] }) {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      for (let i = 0; i < active.length; i++) {
        ...
        try {
          const response = await sendToWhatsApp({ news: item, whatsAppConfig: cfg, useAiTriage });
    ...
    <Card className="mb-6">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center space-x-2">
          <Checkbox id="ai-triage-whatsapp" checked={useAiTriage} onCheckedChange={setUseAiTriage} />
          <Label htmlFor="ai-triage-whatsapp" className="text-sm font-medium">Usar Triagem de IA para aprovar envio</Label>
        </div>

        <Button onClick={handleTest} disabled={isSending || configs.length === 0} className="bg-green-600 hover:bg-green-700">
