...
export default function FeedPage() {
  ...
  const loadData = useCallback(async () => {
    logger.log("[pages/Feed.js]  Carregando noticias...");
    setIsLoading(true);
    ...
    try {
      ...
      logger.log(`[pages/Feed.js]  ${newsData.length} noticias carregadas`);
      ...
      logger.error("[pages/Feed.js]  Erro ao carregar dados:", error);
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleNewsUpdated = useCallback((createdNewsList) => {
    logger.log(`[pages/Feed.js]  Recebidas ${createdNewsList.length} noticias novas para adicionar!`);

    if (!createdNewsList || createdNewsList.length === 0) {
      logger.warn("[pages/Feed.js]  Nenhuma noticia nova recebida");
      return;
    }

    setNews(prevNews => {
      const updatedNews = [...createdNewsList, ...prevNews];
      logger.log(`[pages/Feed.js]  Estado atualizado: ${prevNews.length}  ${updatedNews.length} noticias`);
      return updatedNews;
    });
    ...
    logger.log("[pages/Feed.js]  Noticias adicionadas com sucesso ao feed!");
  }, []);
  ...
  // Sincronizar categoria da URL com estado (permite navegação back/forward)
  useEffect(() => {
  ...
  // Usar ref para evitar memory leak ao recriar interval
  ...
  useEffect(() => {
    if (refreshInterval > 0) {
      const intervalId = setInterval(() => loadDataRef.current(), refreshInterval);
      return () => clearInterval(intervalId);
    }
  }, [refreshInterval]);

  // Memoizar filtros para evitar recálculo desnecessário
  useEffect(() => {
    logger.log(`[pages/Feed.js]  Recalculando filtros. Total de noticias: ${news.length}, Trigger: ${filterTrigger}`);
    ...
    logger.log(`[pages/Feed.js]  Noticias filtradas: ${filtered.length}`);
  ...
  const handleDeleteAllNews = async () => {
    setIsDeleting(true);
    try {
      logger.log("[Feed] Iniciando limpeza completa do feed...");
      const response = await clearAllNews();
      const result = response?.data;

      if (result?.success) {
        logger.log(`[Feed] ${result.deleted_count} noticia(s) removidas do armazenamento local.`);
      ...
      logger.error("[Feed] Erro ao deletar noticias:", error);
  ...
  // Memoizar separação de notícias destacadas para otimizar performance
  const { highlightedNews, regularNews } = useMemo(() => ({
    highlightedNews: filteredNews.filter(item => item.is_highlighted),
    regularNews: filteredNews.filter(item => !item.is_highlighted)
  }), [filteredNews]);
