...
import { Users, Edit, Trash2, Eye, EyeOff, CheckCircle } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function TeamsConfigCard({ config, onEdit, onDelete, onToggleActive }) {
  return (
    <motion.div
    ...
    >
      <Card className={`relative overflow-hidden transition-all duration-300 hover:shadow-lg ${
        config.is_active ? 'border-purple-200 bg-white' : 'border-gray-200 bg-gray-50 opacity-75'
      }`}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <CardTitle className="text-lg font-bold text-gray-900 mb-1">
                  {config.channel_name || "Teams Channel"}
                </CardTitle>
          ...
          <div className="space-y-3">
            {config.team_name && (
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-gray-900 font-medium">{config.team_name}</span>
              </div>
            )}
